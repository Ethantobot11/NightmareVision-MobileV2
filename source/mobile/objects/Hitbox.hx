...
class Hitbox extends FlxSpriteGroup
{
	...
	public function new(?CustomMode:String):Void
	{
		...
		if ((ClientPrefs.hitboxmode != 'New' && ClientPrefs.hitboxmode != 'Classic' && CustomMode == null) || CustomMode != null)
		{
			...
			if (!MobileData.hitboxModes.exists(Custom)) throw 'The Custom Hitbox File doesn\'t exists.';
			...
			for (buttonData in currentHint)
			{
				...
				Reflect.setField(this, buttonData.button, createHint(buttonX, buttonY, buttonWidth, buttonHeight, CoolUtil.colorFromString(buttonColor), customReturn));
	...
	override function destroy():Void
	{
		...
		for (field in Reflect.fields(this))
			if (Std.isOfType(Reflect.field(this, field), MobileButton)) Reflect.setField(this, field, FlxDestroyUtil.destroy(Reflect.field(this, field)));
	}
	...
	private function createHint(X:Float, Y:Float, Width:Int, Height:Int, Color:Int = 0xFFFFFF, ?customReturn:String):MobileButton
	{
		...
		hint.onDown.callback = hint.onOver.callback = function() {
			if (hint.alpha != ClientPrefs.hitboxalpha) hint.alpha = ClientPrefs.hitboxalpha;
		}
