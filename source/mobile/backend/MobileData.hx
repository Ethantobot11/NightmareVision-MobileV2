...
class MobileData
{
	...
	public static function init()
	{
		...
		readDirectory(Paths.getCorePath('mobile/MobileButton/DPadModes'), dpadModes);
		readDirectory(Paths.getCorePath('mobile/Hitbox/HitboxModes'), hitboxModes);
		readDirectory(Paths.getCorePath('mobile/MobileButton/ActionModes'), actionModes);
		#if MODS_ALLOWED
		for (folder in directoriesWithFile(Paths.getCorePath(), 'mobile/MobileButton/'))
		{
	...
	static function directoriesWithFile(path:String, fileToFind:String, mods:Bool = true)
	{
		...
		if(mods)
		{
			...
			{
				var folder:String = Mods.mods(mod + '/' + fileToFind);
				if(FileSystem.exists(folder) && !foldersToCheck.contains(folder)) foldersToCheck.push(folder);
			}

			// Then "PsychEngine/mods/" main folder
			var folder:String = Mods.mods(fileToFind);
			if(FileSystem.exists(folder) && !foldersToCheck.contains(folder)) foldersToCheck.push(Mods.mods(fileToFind));

			// And lastly, the loaded mod's folder
			if(Mods.currentModDirectory != null && Mods.currentModDirectory.length > 0)
			{
				var folder:String = Mods.mods(Mods.currentModDirectory + '/' + fileToFind);
				if(FileSystem.exists(folder) && !foldersToCheck.contains(folder)) foldersToCheck.push(folder);
			}
		}
		#end
